...
#include "UI/optionDesign/drawingFunction.hpp"
...
/*	bool ControlPressed(int control, Lists::ControlFlags flags = Lists::ControlFlags::NONE) {
		switch (flags) {
		case Lists::ControlFlags::NONE:
			return PAD::IS_CONTROL_PRESSED(2, control);
			break;
		case Lists::ControlFlags::ONLY_CONTROLLER:
			if (!PAD::IS_USING_KEYBOARD_AND_MOUSE(2)) {
				return PAD::IS_CONTROL_PRESSED(2, control);
			}
			break;
		case Lists::ControlFlags::ONLY_KEYBOARD:
			if (PAD::IS_USING_KEYBOARD_AND_MOUSE(2)) {
				return PAD::IS_CONTROL_PRESSED(2, control);
			}
			break;
	...
	void Framework::LoadVariablesFromJson(const std::string& filePath) {
		...
		if (file.is_open()) {
			...
			LOG(INFO) << "Theme failed to load; Theme no exist.";
		}
	}
	...
	void Framework::SaveVariablesToJson(std::string name) {
		...
		if (file.is_open()) {
			...
			LOG(INFO) << "Failed to create JSON file.";
		}
	}
	...
	void Framework::HandleInput()
	{
		if (controlsEnabled && !g_Settings.command_window) {
			...
			if (m_OpenKeyPressed && openTimer.Update())
			{
				...
				if (m_Sounds)
					PlaySound2(m_Opened ? "SELECT" : "BACK", "HUD_FRONTEND_DEFAULT_SOUNDSET");
			}
			if (!PAD::IS_USING_KEYBOARD_AND_MOUSE(2)) {
				if (DisabledControlPressed(227) && DisabledControlPressed(Lists::INPUT_CELLPHONE_RIGHT) && openTimer.Update())
				{
					g_HasSetToolTip = false;
					if (!m_Opened) {
						MenuOpeningAnimation();
			...
			{
				if (m_Sounds)
					PlaySound2("BACK", "HUD_FRONTEND_DEFAULT_SOUNDSET");
			...
			if (m_Opened && DisabledControlPressed(194) && backTimer.Update())
			{
				if (m_Sounds)
					PlaySound2("BACK", "HUD_FRONTEND_DEFAULT_SOUNDSET");
			...
			if (m_Opened && !m_SubmenuStack.empty())
			{
				...
				DisableControl(0, true);
				DisableControl(19, true);
				DisableControl(140, true);
				DisableControl(20, true);
				DisableControl(27, true);
				DisableControl(80, true);
				DisableControl(337, true);
				DisableControl(27, true);
				DisableControl(85, true);
				DisableControl(74, true);
				DisableControl(58, true);
				DisableControl(70, true);
				auto sub = m_SubmenuStack.top();
				...
				if (m_EnterKeyPressed && sub->GetNumOptions() != 0 && enterTimer.Update())
				{
					...
					if (m_Sounds)
						PlaySound2("SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET");
				...
				if (DisabledControlPressed(191) && sub->GetNumOptions() != 0 && enterTimer.Update())
				{
					if (g_Settings.m_LockMouse) {
						g_Settings.m_LockMouse = false;
					}
				...
				if (m_UpKeyPressed && sub->GetNumOptions() != 0 && upTimer.Update())
				{
					...
					if (m_Sounds)
						PlaySound2("NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET");
				...
				if (DisabledControlPressed(172) && sub->GetNumOptions() != 0 && upTimer.Update())
				{
					if (g_Settings.m_LockMouse) {
						g_Settings.m_LockMouse = false;
					}
				...
				if (DisabledControlPressed(173) && sub->GetNumOptions() != 0 && downTimer.Update())
				{
					if (g_Settings.m_LockMouse) {
						g_Settings.m_LockMouse = false;
					}
				...
				if (m_LeftKeyPressed && sub->GetNumOptions() != 0 && leftTimer.Update())
				{
					...
					if (auto opt = sub->GetOption(sub->GetSelectedOption()))
					{
						...
						if (m_Sounds && !opt->GetFlag("none", "bool"))
							PlaySound2("NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET");
						opt->HandleAction(OptionAction::Left);
					}
				}
				if (DisabledControlPressed(174) && sub->GetNumOptions() != 0 && leftTimer.Update())
				{
				...
				/*if (DisabledControlPressed(175) && !ControlPressed(227) && sub->GetNumOptions() != 0 && rightTimer.Update())
				{
					if (auto opt = sub->GetOption(sub->GetSelectedOption()))
					{
						if (g_Settings.m_LockMouse) {
	...
	void Framework::DrawOption(OptionBase* opt, bool selected)
	{
		...
		if (opt->GetFlag("number", "bool") || opt->GetFlag("choose", "bool") || opt->GetFlag("none", "bool"))
		{
			if (ytd.toggles) {
				...
				case 0:
					DrawingFunctions()->Sprite("", "", g_Render->m_PosX + (g_Render->m_Width / g_Render->m_OptionPadding - 0.005f), g_Render->m_DrawBaseY + (g_Render->m_OptionHeight / 2.5f) - (DrawingFunctions()->GetTextHeight(g_Render->m_OptionFont, g_Render->m_OptionTextSize) / 1.5f) + 0.014f - 0.001, res.x, res.y, ToggledOn ? m_ToggleOnColor : m_ToggleOffColor, 0.0);
					break;
				case 1:
		...
		if (opt->GetFlag("submenu", "none") || opt->GetFlag("submenu", "player"))
		{
			...
			if (ytd.submenu) {
				...
				case 0:
					...
					break;
				case 1:
					DrawingFunctions()->Rectangle(m_PosX + (m_Width / m_OptionPadding) + 0.00355f + enterable.x_offset, m_DrawBaseY + ((m_OptionHeight) / 2.f), 0.0035f + enterable.width_offset, m_OptionHeight, { m_HeaderBackgroundColor.r, m_HeaderBackgroundColor.g, m_HeaderBackgroundColor.b, m_HeaderBackgroundColor.a });
					break;
				}
			}
		}
