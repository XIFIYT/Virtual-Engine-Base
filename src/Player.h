...
namespace Big {
	...
	public:
		...
		void draw_info4(Ped gaurd) {
			...
			char make[128];
			char make2[128];
			ScriptGlobal globalplayer_bd(2657704);
			ScriptGlobal gpbd_fm_3(1895156);
			ScriptGlobal gpbd_fm_1(1853988);
			Vector3 get_coords = ENTITY::GET_ENTITY_COORDS(gaurd, false);
		...
		netAddress get_ip_address(std::uint32_t player)
		{
			...
			if (player == PLAYER::PLAYER_ID())
				return (*g_GameVariables->m_ped_factory)->m_local_ped->m_player_info->m_net_player_data.m_external_ip;
		...
		uint16_t get_port(std::uint32_t player)
		{
			...
			if (player == PLAYER::PLAYER_ID())
				return (*g_GameVariables->m_ped_factory)->m_local_ped->m_player_info->m_net_player_data.m_external_port;
		...
		void draw_info(std::uint32_t player) {
			try {
				...
				if (hide_information.ped) {
					draw_on_screen_ped(
						NETWORK::NETWORK_IS_SESSION_STARTED() ? get_ped(player) : (*g_GameVariables->m_ped_factory)->m_local_ped->m_player_info->m_ped,
