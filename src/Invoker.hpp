...
extern "C" {
	void _call_asm(void* context, void* function, void* ret);
}
	...
	public:
		...
		bool MapNatives() {
			for (auto const& mapping : g_Crossmap) {
				rage::scrNativeHandler handler = g_GameFunctions->m_GetNativeHandler(
					g_GameFunctions->m_NativeRegistrationTable, mapping.m_New);
		...
		void EndCall(std::uint64_t hash) {
			if (auto cache = m_Cache.find(hash); cache != m_Cache.end()) {
				...
				if (!handler) {
					LOG(INFO) << "Failed to invoke native %p", hash;
					return;
				}
				handler(&m_Context);
				//Fix Vectors
				...
				LOG(INFO) << "Failed to find 0x%p native's handler.", hash;
			}
		}
	private:
